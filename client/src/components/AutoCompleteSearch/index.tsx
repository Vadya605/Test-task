...
import { AutoCompeteSearchWrapper, SearchBox, SearchIcon, SearchInput } from './styled'
...
export default function AutoCompleteSearch() {
    ...
    const { ready, value, suggestions, setValue, init, clearSuggestions } = usePlacesAutocomplete({
        initOnMount: false,
        debounce: 300,
    })
    ...
    return (
        <AutoCompeteSearchWrapper data-testid="auto-compete-search" ref={ref}>
            <SearchBox isActive={status === STATUS_CODES.OK}>
                <SearchIcon />
                <SearchInput placeholder="Место адрес..." value={value} onChange={handleInput} disabled={!ready} />
            </SearchBox>
            {status === STATUS_CODES.OK && (
                <AutoCompleteSuggestions suggestions={data} handleSelectSuggestion={handleSelect} />
            )}
